...
top_japanese_employer = selected_rows.iloc[0]["company"]
...
for word in countries:
    selected_rows = f500[f500["country"] == word]
    selected_rows_value = selected_rows.sort_values("employees", ascending=False)
    top_employer = selected_rows_value.iloc[0]
    employer_name = top_employer["company"]
    top_employer_by_country[word] = employer_name
...
for sector in f500["sector"].unique():
    sector_name = f500["sector"] == sector
    sector_companies = f500.loc[sector_name]
